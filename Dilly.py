...
class Cancel(discord.ui.Button):
    ...
    async def callback(self, interaction):
        user_data = db.verify.find_one({"PlrName": self.RobloxID})
        rblxId = user_data.get('PlrName') if user_data else None
        didName = user_data.get("DiscordName") if user_data else None
        print(didName)

        if user_data:
            # 문서가 삭제되었을 때 추가 동작 수행
            await interaction.response.send_message(f"{rblxId}님의 인증이 취소되었어요.\n재인증을 시도하시려면, `/딜리계좌만들기` 명령어를 이용해주세요!", ephemeral=True)
            db.verify.delete_one({"PlrName": rblxId})
            Button.disabled = False
        else:
            await interaction.response.send_message("취소해드릴 인증요청이 없어요.\n문제가 지속적으로 반복된다면, 문의하기 채널을 이용해주세요.", ephemeral=True)
...
async def pay(interaction: discord.Interaction, 로블록스닉네임: str):
    ...
    DicodName = db.PayNumber.find_one({"discordId": str(interaction.user.id)})
    ...
    if DicodName :
        ...
        if user_data:
            embed = discord.Embed(colour=discord.Colour.red(), title="오류가 발생했어요", description=f"{로블록스닉네임}(으)로 만들어진 계좌가 이미 존재합니다.\n계좌번호를 조회하시려면 `/내계좌번호확인하기` 명령어를 이용해주세요!")
            view = discord.ui.View()
